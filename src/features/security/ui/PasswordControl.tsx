...
function PasswordControl(): ReactElement {
  ...
  return (
    <Disclosure>
      {({ open, close }) => {
        const onSubmit = ({ password, old_password }: ChangePasswordValidationSchema) => {
        ...
        return (
          <form onSubmit={(data) => void handleSubmit(onSubmit)(data)} className="flex flex-col items-start min-w-[480px]">
            <Disclosure.Button as="div">
              <Button
                type="button"
                extraClass={`text-[18px] font-semibold ${open ? "!text-secondary-600" : "!text-main-violet-600"}`}
