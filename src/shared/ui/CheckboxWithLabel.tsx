  ...
  return (
    <div className={`flex items-center ${extraBoxClass}`}>
      <input
        ...
        type="checkbox"
        className="relative h-5 w-5 m-0.5 cursor-pointer appearance-none outline-none rounded-[5px] border-2 border-text-black bg-center bg-no-repeat checked:bg-check"
        checked={isChecked}
      />
      <label
        htmlFor={id}
        className={`text-[18px] cursor-pointer ${extraLabelClass}`}
      >{label}</label>
    </div>
  )
}
