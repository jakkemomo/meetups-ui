    ...
    <Disclosure>
      {
        ({ open, close }) => (
          <>
            ...
            />
            <Disclosure.Button className={`flex items-center text-[18px] leading-[22.59px] py-[10px] w-full text-start px-[22px] mt-[7px] ${isPeriodic ? "bg-secondary-100" : "text-white bg-secondary-300"} ${open ? "rounded-t-[10px] pt-[20px]" : "rounded-[10px]"} ${error ? open ? "border-solid border-system-500 border-t-1 border-x-1" : "border-solid border-system-500 border-1" : ""}`} disabled={!isPeriodic}>
              Настройки
            ...
            <Disclosure.Panel className={`flex pt-[7px] px-[22px] pb-5 bg-secondary-100 rounded-b-[10px] ${error ? "border-solid border-system-500 border-b-1 border-x-1" : ""}`}>
              ...
              <ul className="flex items-center ml-[18px]">
                {
                  daysArr.map((el, index) => (
                    <li key={index} className="h-[77px] w-[55px] flex flex-col items-center ml-1 first-of-type:ml-0">
                      <div onClick={() => onSelectDay(el)} className={`flex items-center justify-center w-[45px] h-[45px] border-1 border-main-violet-600 border-solid rounded-[10px] cursor-pointer ${schedule.some((item) => item.day_of_week === el.day_of_week) ? "bg-main-violet-600 text-white" : ""}`}>
                      ...
                      </div>
                      {
                        schedule.some((item) => item.day_of_week === el.day_of_week) && (
                          <TimeInput
                            ...
                            extraInputClass={`${isInputsDisabled ? "text-text-black" : "text-secondary-100"}`}
