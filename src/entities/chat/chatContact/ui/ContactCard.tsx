...
function ContactCard({ data, onClick, isSelected }: IContactCard): ReactElement {
  return (
    <div className={`flex pr-[11px] cursor-pointer ${isSelected ? 'bg-gradient-to-l from-custom-gray rounded-lg' : ''}`} onClick={onClick}>
      ...
      <div className="w-full flex flex-col ml-[22px] mt-2.5">
        <div className="flex w-full justify-between">
          ...
          <p className="text-[14px] text-placeholder-gray">1 марта</p>
        </div>
        <div className="flex items-center justify-between mt-2.5">
          <p className="text-[14px] text-[#616161] leading-[18px] truncate max-w-[280px]">{data.last_message_text}</p>
          <div className="rounded-circle bg-but-primary w-4 h-4 flex items-center justify-center text-white text-[12px]">2</div>
        </div>
      </div>
    </div>
  )
